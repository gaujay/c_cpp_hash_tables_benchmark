...
#else
  #define INDIVI_UTABLE_OFLW_SAT_ASSERT(x)
#endif
...
#else
  #define INDIVI_UTABLE_DIST_SAT_ASSERT(x)
#endif
...
struct alignas(32) MetaGroup // actually aligned/init manually, see NewStorage
{
  ...
  static inline MetaGroup* empty_group() noexcept
  {
...
private:
  ...
  size_type mShift   = 0u;  // hash shift to keep highest bits (as group index)
  size_type mGMask   = 0u;  // group index mask (as power-of-2 capacity - 1)
  size_type mMaxSize = 0u;  // max size before rehash is needed
  Groups mGroups;           // contains hash fragments and metadata (processed as 16-bytes groups)
...
private:
  // static functions
  static constexpr size_type hash_shift(size_type gCapa) noexcept
  {
    INDIVI_UTABLE_ASSERT(is_pow2(gCapa));
    return (gCapa <= 2u) ? 1u // fake value, overridden by mask
                         : sizeof(size_type) * CHAR_BIT - (size_type)(first_bit_index(gCapa));
  }

  static constexpr size_type hash_position(std::size_t hash, size_type shift, size_type mask) noexcept
  {
    return (hash >> shift) & mask; // keep (64 - n) highest bits, mask to handle single group case
  }
  ...
  // member functions
#if defined(INDIVI_FLAT_U_DEBUG) && !defined(NDEBUG)
  ...
  Location unchecked_insert(std::size_t hash, size_type gIndex, U&& value)
  {
    ...
    do {
      ...
      if (empties) // not full
      {
        int idx = first_bit_index(empties);
  ...
  template< typename U >
  void insert_unique(MetaGroup* groups, item_type* values, size_type shift, size_type gMask, U&& value)
  {
    std::size_t hash = get_hash(get_key(value));
    size_type gIndex = hash_position(hash, shift, gMask);
  ...
  template< typename U >
  Location insert_first(MetaGroup* groups, item_type* values, size_type shift, size_type gMask, std::size_t hash, U&& value)
  {
    size_type gIndex = hash_position(hash, shift, gMask);
  ...
  template< typename U, class... Args >
  Location emplace_first(MetaGroup* groups, item_type* values, size_type shift, size_type gMask, std::size_t hash, U&& key, Args&&... args)
  {
    size_type gIndex = hash_position(hash, shift, gMask);
  ...
  void fast_copy(const flat_utable& other)
  {
    ...
    INDIVI_UTABLE_ASSERT(mValues.data);
    INDIVI_UTABLE_ASSERT(mShift == other.mShift);
  ...
  void rehash_impl(size_type newCapa)
  {
    ...
    size_type newShift = hash_shift(newGCapa);
    ...
    {
      INDIVI_UTABLE_ASSERT(mSize == 0u);
      INDIVI_UTABLE_ASSERT(mShift == 0u);
